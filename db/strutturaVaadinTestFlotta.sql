...
-- Dump della struttura di tabella vaadinflottatest.alimentazioni
DROP TABLE IF EXISTS `alimentazioni`;
CREATE TABLE IF NOT EXISTS `alimentazioni` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `nome_tipologia` varchar(50) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `modelli` (
  ...
  `id_alimentazione` int(11) NOT NULL DEFAULT 0,
  ...
  KEY `id_alimentazione` (`id_alimentazione`),
  CONSTRAINT `id_alimentazione` FOREIGN KEY (`id_alimentazione`) REFERENCES `alimentazioni` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
